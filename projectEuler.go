...
var Arr = [20][20]int{
	...
	a, b := 1, 2
	fmt.Println(Arr[a][b])
	fmt.Println(Arr[a][b], Arr[a][b+1], Arr[a][b+2], Arr[a][b+3])
	fmt.Println(Arr[a][b], Arr[a+1][b], Arr[a+2][b], Arr[a+3][b])
	fmt.Println(Arr[a][b], Arr[a+1][b+1], Arr[a+2][b+2], Arr[a+3][b+3])
	fmt.Println(Arr[a][b+3], Arr[a+1][b+2], Arr[a+2][b+1], Arr[a+3][b])
	prod := 1
...
problem_0 one is a Python one liner: 
sum(i for i in range(maxnum) if i % 3 == 0 or i % 5 == 0)
Amusing misuse of whitespace
